...
export function ContactSection() {
    ...
    },
    {
      ...
      icon: (
        <img
          ...
          alt="Medium"
          className="w-4 h-4 md:w-5 md:h-5 lg:w-6 lg:h-6 object-contain filter grayscale invert group-hover:invert-0 transition-all duration-[1000ms]"
        />
      ),
    },
    {
  ...
  return (
    <section
      id="contact"
      className="relative min-h-[100vh] w-full flex flex-col items-center justify-center -mt-32 px-6 overflow-hidden"
    >
      ...
      <motion.h2
        className="relative z-10 text-5xl md:text-8xl lg:text-[10rem] font-black tracking-tighter mb-24 text-center leading-none select-none flex flex-wrap justify-center gap-x-4 md:gap-x-8"
        initial={{ opacity: 0, scale: 0.9 }}
        whileInView={{ opacity: 1, scale: 1 }}
        transition={heavySpring}
        viewport={{ once: true, margin: "-100px" }}
      >
        {["Build", "the", "future."].map((word, wordIdx) => (
          <span key={wordIdx} className="inline-flex whitespace-nowrap">
            {word.split("").map((char, charIdx) => (
              <motion.span
                ...
                className="inline-block cursor-grab active:cursor-grabbing text-white hover:text-white transition-colors"
              >
                {char}
              </motion.span>
            ))}
      ...
      >
        {socials.map((social, i) => (
          <motion.a
            ...
            className="w-10 h-10 md:w-14 md:h-14 lg:w-16 lg:h-16 rounded-full border border-white/20 flex items-center justify-center hover:bg-white hover:text-black hover:border-white hover:scale-115 transition-all duration-300 ease-out group cursor-none relative text-white"
          ...
          >
            ...
            {/* Tooltip on hover */}
            <span className="absolute -bottom-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-xs text-zinc-400 uppercase tracking-widest font-light pointer-events-none whitespace-nowrap">
              {social.name}
            </span>
          </motion.a>
        ))}
